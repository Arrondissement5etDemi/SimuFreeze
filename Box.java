 ...
 * 	@return Box, the constructed box */
	public Box(int nGiven, double dGiven, double co) {
		...
		while (i < n) {
			...
			//check the distance from particle p to other generated particles
			boolean pass = true;
			for (int j = 0; j < i; j++) {
				double distPJ = p.distanceto(partiArr[j]);
				if (distPJ < minNeighborDist) { 
					pass = false;
					break;
				}
			}
